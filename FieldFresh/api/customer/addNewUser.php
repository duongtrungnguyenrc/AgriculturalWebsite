<?php
    ...
    if (!$error) {
        ...
        if ($interval->format('%R') === '+' && $interval->format('%a') >= 1) {
            ...
            $error = true;
            $error_message = "Confirm password must match with your password!";
        }
    }
    ...
    $sever = new Sever();
    echo $sever->addNewUser($data['userName'], $data['password'], $data['fullName'], $data['birth'], $data['gender'], $data['email'], $data['phone'], $data['address']);
?>